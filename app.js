...
app.get("/get-skus-by-status", async (req, res) => {
  ...
  if (!branch || !statusCategory || !status) {
    return res
      .status(400)
      .json({ message: "Branch, Category, and Status are required" });
  }
  ...
  try {
    ...
    const skus = branchData.SKUs.filter(
      (sku) => !sku.enabled && sku.status === status
    );
...
app.post("/disable-sku", async (req, res) => {
  ...
  try {
    ...
    if (!branchSKU) {
      return res
        .status(404)
        .json({ message: "Branch and category not found." });
    }
    ...
    if (!sku) {
      return res
        .status(404)
        .json({ message: "SKU not found in this category." });
    }
    ...
    res
      .status(500)
      .json({ message: "An error occurred while updating SKU status." });
  }
});
  ...
  try {
    ...
    if (!sku.enabled && ["Not Carried", "Delisted"].includes(sku.status)) {
      ...
      return res.status(400).json({
        message:
          "SKU is already enabled or does not meet the criteria for enabling.",
      });
    }
...
app.post("/delisted-sku", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ message: "SKU status updated to Delisted successfully." });
  } catch (error) {
    console.error("Error updating SKU status:", error);
    res
      .status(500)
...
app.post("/update-sku-status", async (req, res) => {
  ...
  try {
    ...
    sku.status = status;
    sku.enabled =
      status === "Not Carried" || status === "Delisted" ? false : true; // Update SKU enabled state based on status
...
app.post("/save-branch-sku", async (req, res) => {
  try {
    ...
    res
      .status(201)
      .json({ message: "BranchSKUs saved successfully", data: result });
  } catch (error) {
